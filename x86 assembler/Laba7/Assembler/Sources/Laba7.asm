...
enterAB:
	...
	fld A ;Загрузка числа из памяти в вершину стека сопроцессора
	fld B ; Загрузка числа из памяти в вершину стека сопроцессора
	fcom ; Сравниваем вещественные числа
	fstsw AX   ;копирует текущий статус сопроцессора, включая флаги условий, в регистр AX
	sahf ;загружает регистр флагов процессора значениями из регистра AH
	jb enterAB 
...
checkRoot: ;проверка на сходимость f(a)*f(b)<0
	...
	fst y1y2 
	fcom
	fstsw ax ;Сохраняет состояние статуса сопроцессора в регистр ax
	sahf 
	ja endNoRoot ;Переходит к метке endNoRoot, если ST(0) больше ST(1)
...
; Подпрограмма, выполняющая вычисление функции f(x)=x^3–7x^2–9x+49
;Действие: 
...
StrToDouble proc near
	...
	push ax bx dx cx di
	pushf
	mov word ptr[bp - 2], 10 ;Резервируется два байта для временного хранения числа 10
	fild word ptr[bp - 2] 
	fldz ;Загрузить 0
	mov di, 0
	mov bx, [bp + 4] 
	cmp byte ptr[bx], '-' ;Проверяется, является ли первый символ строки минусом, указывающим на отрицательное число.
	jne @@BPN
	inc bx
	mov di, 1
...
@@BPN: 
	...
	movsx ax, byte ptr [bx];Загружаем текущий символ строки в регистр
	cmp ax, '.'
	je @@PNT1
	cmp ax, 0dh 
	jne @@CNT
	fxch st(1)
	fstp st(0) ;Сохранение числа из вершины стека сопроцессора в память с последующим выталкиванием его из вершины стека
	jmp @@REN
...
@@APN: 
	...
	mov word ptr[bp - 2], ax
	fiadd word ptr[bp - 2] 
	fdiv st(0), st(1) ;Делит значение ST(0) на ST(1). Результат помещается в ST(0)
	dec bx
	jmp @@APN
...
@@REN:
	cmp di, 1 ;Проверяем, было ли изначально число отрицательным.
	jne @@CYK
	fchs ;Инвертируем
